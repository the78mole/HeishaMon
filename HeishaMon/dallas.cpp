...
#define MAXTEMPDIFFPERSEC                                                      \
  0.5 // what is the allowed temp difference per second which is allowed (to
      // filter bad values)
...
void initDallasSensors(void (*log_message)(char *),
                       unsigned int updateAllDallasTimeSettings,
                       unsigned int dallasTimerWaitSettings,
                       unsigned int dallasResolution) {
  char log_msg[256];
  ...
  sprintf_P(log_msg, PSTR("Number of 1wire sensors on bus: %d"),
            dallasDevicecount);
  log_message(log_msg);
  if (dallasDevicecount > MAX_DALLAS_SENSORS) {
    dallasDevicecount = MAX_DALLAS_SENSORS;
    sprintf_P(log_msg,
              PSTR("Reached max 1wire sensor count. Only %d sensors will "
                   "provide data."),
              dallasDevicecount);
    log_message(log_msg);
  }
  ...
  for (int i = 0; i < dallasDevicecount; i++) {
    ...
    for (int x = 0; x < 8; x++) {
      ...
      sprintf(&actDallasData[i].address[x * 2], "%02x",
              actDallasData[i].sensor[x]);
    }
    sprintf_P(log_msg, PSTR("Found 1wire sensor: %s"),
              actDallasData[i].address);
    log_message(log_msg);
  }
  if (DALLASASYNC)
...
void resetlastalldatatime_dallas() { lastalldatatime_dallas = 0; }
...
void readNewDallasTemp(PubSubClient &mqtt_client, void (*log_message)(char *),
  ...
  if ((lastalldatatime_dallas == 0) ||
      ((unsigned long)(millis() - lastalldatatime_dallas) >
       (1000 * updateAllDallasTime))) {
    updatenow = true;
  ...
  for (int i = 0; i < dallasDevicecount; i++) {
    ...
    if (temp < -120.0) {
      sprintf_P(log_msg, PSTR("Error 1wire sensor offline: %s"),
                actDallasData[i].address);
      log_message(log_msg);
    } else {
      float allowedtempdiff =
          (((millis() - actDallasData[i].lastgoodtime)) / 1000.0) *
          MAXTEMPDIFFPERSEC;
      if ((actDallasData[i].temperature != -127.0) and
          ((temp > (actDallasData[i].temperature + allowedtempdiff)) or
           (temp < (actDallasData[i].temperature - allowedtempdiff)))) {
        sprintf_P(log_msg,
                  PSTR("Filtering 1wire sensor temperature (%s). Delta to "
                       "high. Current: %.2f Last: %.2f"),
                  actDallasData[i].address, temp, actDallasData[i].temperature);
        log_message(log_msg);
      } else {
        actDallasData[i].lastgoodtime = millis();
        if ((updatenow) || (actDallasData[i].temperature !=
                            temp)) { // only update mqtt topic if temp changed
                                     // or after each update timer
          actDallasData[i].temperature = temp;
          sprintf(log_msg, PSTR("Received 1wire sensor temperature (%s): %.2f"),
                  actDallasData[i].address, actDallasData[i].temperature);
          log_message(log_msg);
          if (true) {
            sprintf_P(valueStr, PSTR("%.2f"), actDallasData[i].temperature);
            sprintf_P(mqtt_topic, PSTR("%s/%s/%s"), mqtt_topic_base,
                      mqtt_topic_1wire, actDallasData[i].address);
            mqtt_client.publish(mqtt_topic, valueStr, MQTT_RETAIN_VALUES);
            sprintf_P(valueStr, PSTR("%s"), actDallasData[i].alias);
            sprintf_P(mqtt_topic, PSTR("%s/%s/%s/alias"), mqtt_topic_base,
                      mqtt_topic_1wire, actDallasData[i].address);
            mqtt_client.publish(mqtt_topic, valueStr, MQTT_RETAIN_VALUES);
          } else {
            sprintf_P(valueStr, PSTR("{\"Temperature\":%.2f,\"Alias\":\"%s\"}"),
                      actDallasData[i].temperature, actDallasData[i].alias);
            sprintf_P(mqtt_topic, PSTR("%s/%s/%s"), mqtt_topic_base,
                      mqtt_topic_1wire, actDallasData[i].address);
            mqtt_client.publish(mqtt_topic, valueStr, MQTT_RETAIN_VALUES);
          }
          sprintf_P(log_msg,
                    PSTR("{\"data\": {\"dallasvalues\": {\"sensorID\": \"%s\", "
                         "\"value\": %.2f}}}"),
                    actDallasData[i].address, actDallasData[i].temperature);
...
void dallasJsonOutput(struct webserver_t *client) {
  ...
  for (int i = 0; i < dallasDevicecount; i++) {
    ...
    webserver_send_content(client, actDallasData[i].address,
                           strlen(actDallasData[i].address));
    ...
    webserver_send_content(client, actDallasData[i].alias,
                           strlen(actDallasData[i].alias));
  ...
  if (LittleFS.begin()) {
    if (LittleFS.exists("/dallas.json")) {
      ...
      if (configFile) {
        ...
        if (!error) {
          for (int i = 0; i < dallasDevicecount; i++) {
            if (jsonDoc[actDallasData[i].address])
              strncpy(actDallasData[i].alias, jsonDoc[actDallasData[i].address],
                      sizeof(actDallasData[i].alias));
          }
        }
      }
    }
