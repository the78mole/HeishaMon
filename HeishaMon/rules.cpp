...
#if defined(ESP8266)
#define MEMPOOL_ADDRESS 0x40280000 // ESP8266 memory address
...
// if((rules[i]->ast.buffer = (unsigned char *)MALLOC(rules[i]->ast.bufsize)) ==
// NULL) { OUT_OF_MEMORY
// }
...
static int8_t is_variable(char *text, uint16_t size) {
  ...
  if (size == strlen_P(PSTR("ds18b20#2800000000000000")) &&
      strncmp_P(text, PSTR("ds18b20#"), 8) == 0) {
    return 24;
    ...
    if (text[0] == '@') {
      ...
      int nroptcommands =
          sizeof(optionalCommands) / sizeof(optionalCommands[0]);
    ...
    if (text[0] == '?') {
      ...
      while (heishaOTDataStruct[x].name != NULL) {
        ...
        if (size - 1 == len &&
            strnicmp(&text[1], heishaOTDataStruct[x].name, len) == 0) {
          i = len + 1;
          match = 1;
          break;
        }
...
static int8_t is_event(char *text, uint16_t size) {
  ...
  if (size == strlen_P(PSTR("ds18b20#2800000000000000")) &&
      strncmp_P((const char *)text, PSTR("ds18b20#"), 8) == 0) {
    return 24;
  }
...
static void rule_done_cb(struct rules_t *obj) { return; }
...
static int8_t vm_value_get(struct rules_t *obj) {
  ...
  if (key[0] == '?') {
    ...
    for (i = 0; i < NUMBER_OF_OPT_TOPICS; i++) {
      ...
      if (stricmp(cpy, (char *)&key[1]) == 0) {
        String dataValue =
            actOptData[0] == '\0' ? "" : getOptDataValue(actOptData, i);
        char *str = (char *)dataValue.c_str();
        if (strlen(str) == 0) {
          rules_pushnil(obj);
        } else if (check_is_number(str) == 0) {
    ...
    for (i = 0; i < NUMBER_OF_TOPICS_EXTRA; i++) {
      ...
      if (stricmp(cpy, (char *)&key[1]) == 0) {
        String dataValue =
            actDataExtra[0] == '\0' ? "" : getDataValueExtra(actDataExtra, i);
        char *str = (char *)dataValue.c_str();
        if (strlen(str) == 0) {
          rules_pushnil(obj);
        } else if (check_is_number(str) == 0) {
  ...
  if (key[0] == '@') {
    ...
    if (parsing == 0 && !heishamonSettings.listenonly) {
      ...
      if (heishamonSettings.optionalPCB) {
        // optional commands
        for (uint8_t x = 0;
             x < sizeof(optionalCommands) / sizeof(optionalCommands[0]); x++) {
          optCmdStruct tmp;
    ...
    if (key[0] == '$') {
      table = (struct varstack_t *)obj->userdata;
      if (table == NULL) {
        if ((table = (struct varstack_t *)MALLOC(sizeof(struct varstack_t))) ==
            NULL) {
          OUT_OF_MEMORY
        }
    ...
    if (array == NULL) {
      if ((table->array = (struct array_t *)REALLOC(
               table->array, sizeof(struct array_t) * (table->nr + 1))) ==
          NULL) {
        OUT_OF_MEMORY
      }
...
void rules_timer_cb(int nr) {
  ...
  if (nr > -1) {
    ...
    if (ret == 0) {
      logprintf_P(F("%s%d %s %d %s"), F("rule #"), rules[nr]->nr,
                  F("was executed in"), timestamp.second - timestamp.first,
                  F("microseconds"));
...
void rules_setup(void) {
  ...
  if (rule_options.event_cb == NULL) { // check if not initialized before
    ...
    if (mempool == NULL) { // make sure we only malloc if not done before
      mempool = (unsigned char *)ps_malloc(
          MEMPOOL_SIZE); // in arduino IDE normal malloc causes big block to go
                         // to PSRAM if PSRAM is enabled. But seems to be
                         // unstable so for now don't enable PSRAM
      if (mempool == NULL) {
...
#define BUFFER_SIZE 128
    ...
    while (chunk * BUFFER_SIZE < len) {
      ...
      memcpy(&mempool[txtoffset + (chunk * BUFFER_SIZE)], &content,
             alignedbuffer(len1));
      chunk++;
    }
    frules.close();
